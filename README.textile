...
FiscalDate fiscalDate = factory.create(calendarDate);

p. Query the newly created @FiscalDate@ to retrieve the usual suspects of attributes:

bc.. fiscalDate.getFiscalYear();
fiscalDate.getFiscalMonth();
fiscalDate.getFiscalDayOfYear();
fiscalDate.getFiscalWeekOfYear();

fiscalDate.getCalendarYear();
fiscalDate.getCalendarMonth();
fiscalDate.getCalendarDayOfMonth();
fiscalDate.getCalendarDayOfYear();
fiscalDate.getCalendarWeekOfWeekyear();

p. Do some math with dates as follows:

bc. fiscalDate.plusYears(years);
fiscalDate.plusMonths(months);
...
p. All classes are immutable and therefore thread-safe. The library currently relies on "Joda Time":http://joda-time.sourceforge.net/ but will be updated to use the new "JSR 310":http://jcp.org/en/jsr/detail?id=310 based Date API. 
